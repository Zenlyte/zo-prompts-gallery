...
- [ ] Phase 1: Global Management UI Component
    - [ ] Add gear icon to header in `src/pages/demos/skills-gallery.tsx`
...
## Phase 1: Global Management UI Component
**Affected Files:**
- `src/pages/demos/skills-gallery.tsx`: Add Gear icon and management dialog markup.

### Changes
#### `src/pages/demos/skills-gallery.tsx`
- Insert a `<Button variant="ghost" size="icon">` with a `Settings` icon next to the "Skills Gallery" title.
...
## Phase 2: Batch Operation Logic & Conflict Handling
**Affected Files:**
- `src/pages/demos/skills-gallery.tsx`: Implement API integration and conflict UI.

### Changes
#### `src/pages/demos/skills-gallery.tsx`
- Implement `handleBatchPreview(op, from, to)`:
    - Calls `POST /api/skills/batch/preview`.
...
- Implement `handleBatchApply()`:
    - Calls `POST /api/skills/batch/apply`.
    - On success, trigger `loadSkills()` to refresh the entire gallery state.
